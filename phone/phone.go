...
type Modem struct {
	...
	Port              *serial.Port
	mu                sync.Mutex
	cmdMutex          sync.Mutex // ⬅️ NEW: Guards command sessions
...
func (m *Modem) send(cmd string) (string, error) {
	...
	m.cmdWait.Wait()
	return m.lastResponse.String(), nil
}
...
func (m *Modem) Answer() error            { resp, err := m.send("ATA"); m.HandleEvent(resp); return err }
func (m *Modem) Hangup() error            { resp, err := m.send("AT+CHUP"); m.HandleEvent(resp); return err }
...
func (m *Modem) handleCarrierInfo(line string) {
	re := regexp.MustCompile(`\+COPS:\s*(\d+),0,"(.*?)",(\d+)`)
	...
	if len(matches) == 4 {
		...
		m.Carrier = strings.TrimRight(carrier, " ")

		log.Printf("📶 Connected to %s [%s]!", carrier, modes[mode])
	}
}
...
func Run() *Modem {
	modem, err := NewModem("/dev/ttyUSB2", 921600) // Adjust to match your modem's port
	if err != nil {
		log.Println(err)
		return nil
	}
	...
	// Ask the modem what signal quality we have right now
	network_status, _ := modem.send("AT+CSQ")
	modem.HandleEvent(network_status)

	// Ask the modem what network we are on right now
	network_info, _ := modem.send("AT+COPS?")
	modem.HandleEvent(network_info)

	// Ask the modem what network generation we're using right now
	network_gen, _ := modem.send("AT+CNSMOD?")
	modem.HandleEvent(network_gen)
