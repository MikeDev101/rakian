package gfx

import (
	"fmt"
	"image"
)

var Font_large_bold_rune_map = map[rune]string{
	' ':  "0020",
	'!':  "0021",
	'"':  "0022",
	'#':  "0023",
	'$':  "0024",
	'%':  "0025",
	'&':  "0026",
	'\'': "0027",
	'(':  "0028",
	')':  "0029",
	'*':  "002a",
	'+':  "002b",
	',':  "002c",
	'-':  "002d",
	'.':  "002e",
	'/':  "002f",
	'0':  "0030",
	'1':  "0031",
	'2':  "0032",
	'3':  "0033",
	'4':  "0034",
	'5':  "0035",
	'6':  "0036",
	'7':  "0037",
	'8':  "0038",
	'9':  "0039",
	':':  "003a",
	';':  "003b",
	'<':  "003c",
	'=':  "003d",
	'>':  "003e",
	'?':  "003f",
	'@':  "0040",
	'A':  "0041",
	'B':  "0042",
	'C':  "0043",
	'D':  "0044",
	'E':  "0045",
	'F':  "0046",
	'G':  "0047",
	'H':  "0048",
	'I':  "0049",
	'J':  "004a",
	'K':  "004b",
	'L':  "004c",
	'M':  "004d",
	'N':  "004e",
	'O':  "004f",
	'P':  "0050",
	'Q':  "0051",
	'R':  "0052",
	'S':  "0053",
	'T':  "0054",
	'U':  "0055",
	'V':  "0056",
	'W':  "0057",
	'X':  "0058",
	'Y':  "0059",
	'Z':  "005a",
	'\\': "005c",
	'_':  "005f",
	'a':  "0061",
	'b':  "0062",
	'c':  "0063",
	'd':  "0064",
	'e':  "0065",
	'f':  "0066",
	'g':  "0067",
	'h':  "0068",
	'i':  "0069",
	'j':  "006a",
	'k':  "006b",
	'l':  "006c",
	'm':  "006d",
	'n':  "006e",
	'o':  "006f",
	'p':  "0070",
	'q':  "0071",
	'r':  "0072",
	's':  "0073",
	't':  "0074",
	'u':  "0075",
	'v':  "0076",
	'w':  "0077",
	'x':  "0078",
	'y':  "0079",
	'z':  "007a",
	'¡':  "00a1",
	'£':  "00a3",
	'¤':  "00a4",
	'¥':  "00a5",
	'§':  "00a7",
	'º':  "00ba",
	'¿':  "00bf",
	'À':  "00c0",
	'Á':  "00c1",
	'Â':  "00c2",
	'Ã':  "00c3",
	'Ä':  "00c4",
	'Å':  "00c5",
	'Æ':  "00c6",
	'Ç':  "00c7",
	'È':  "00c8",
	'É':  "00c9",
	'Ê':  "00ca",
	'Ë':  "00cb",
	'Ì':  "00cc",
	'Í':  "00cd",
	'Î':  "00ce",
	'Ï':  "00cf",
	'Ñ':  "00d1",
	'Ò':  "00d2",
	'Ó':  "00d3",
	'Ô':  "00d4",
	'Ö':  "00d6",
	'Ø':  "00d8",
	'Ù':  "00d9",
	'Ú':  "00da",
	'Û':  "00db",
	'Ü':  "00dc",
	'ß':  "00df",
	'à':  "00e0",
	'á':  "00e1",
	'â':  "00e2",
	'ã':  "00e3",
	'ä':  "00e4",
	'å':  "00e5",
	'æ':  "00e6",
	'ç':  "00e7",
	'è':  "00e8",
	'é':  "00e9",
	'ê':  "00ea",
	'ë':  "00eb",
	'ì':  "00ec",
	'í':  "00ed",
	'î':  "00ee",
	'ï':  "00ef",
	'ñ':  "00f1",
	'ò':  "00f2",
	'ó':  "00f3",
	'ô':  "00f4",
	'ö':  "00f6",
	'ø':  "00f8",
	'ù':  "00f9",
	'ú':  "00fa",
	'û':  "00fb",
	'ü':  "00fc",
	'ý':  "00fd",
	'Ğ':  "011e",
	'ğ':  "011f",
	'İ':  "0130",
	'ı':  "0131",
	'Œ':  "0152",
	'œ':  "0153",
	'Γ':  "0393",
	'Δ':  "0394",
	'Θ':  "0398",
	'Λ':  "039b",
	'Ξ':  "039e",
	'Π':  "03a0",
	'Σ':  "03a3",
	'Φ':  "03a6",
	'Ψ':  "03a8",
	'Ω':  "03a9",
	'Ѐ':  "0400",
	'Ё':  "0401",
	'Ђ':  "0402",
	'Ѓ':  "0403",
	'Є':  "0404",
	'Ѕ':  "0405",
	'І':  "0406",
	'Ї':  "0407",
	'Ј':  "0408",
	'Љ':  "0409",
	'Њ':  "040a",
	'Ћ':  "040b",
	'Ќ':  "040c",
	'Ѝ':  "040d",
	'Ў':  "040e",
	'Џ':  "040f",
	'А':  "0410",
	'Б':  "0411",
	'В':  "0412",
	'Г':  "0413",
	'Д':  "0414",
	'Е':  "0415",
	'Ж':  "0416",
	'З':  "0417",
	'И':  "0418",
	'Й':  "0419",
	'К':  "041a",
	'Л':  "041b",
	'М':  "041c",
	'Н':  "041d",
	'О':  "041e",
	'П':  "041f",
	'Р':  "0420",
	'С':  "0421",
	'Т':  "0422",
	'У':  "0423",
	'Ф':  "0424",
	'Х':  "0425",
	'Ц':  "0426",
	'Ч':  "0427",
	'Ш':  "0428",
	'Щ':  "0429",
	'Ъ':  "042a",
	'Ы':  "042b",
	'Ь':  "042c",
	'Э':  "042d",
	'Ю':  "042e",
	'Я':  "042f",
	'а':  "0430",
	'б':  "0431",
	'в':  "0432",
	'г':  "0433",
	'д':  "0434",
	'е':  "0435",
	'ж':  "0436",
	'з':  "0437",
	'и':  "0438",
	'й':  "0439",
	'к':  "043a",
	'л':  "043b",
	'м':  "043c",
	'н':  "043d",
	'о':  "043e",
	'п':  "043f",
	'р':  "0440",
	'с':  "0441",
	'т':  "0442",
	'у':  "0443",
	'ф':  "0444",
	'х':  "0445",
	'ц':  "0446",
	'ч':  "0447",
	'ш':  "0448",
	'щ':  "0449",
	'ъ':  "044a",
	'ы':  "044b",
	'ь':  "044c",
	'э':  "044d",
	'ю':  "044e",
	'я':  "044f",
	'ѐ':  "0450",
	'ё':  "0451",
	'є':  "0454",
	'і':  "0456",
	'ї':  "0457",
	'€':  "20ac",
	'':  "e000",
}

func load_font_large_bold_rune(char rune) (*image.Image, error) {
	relPath, ok := Font_large_bold_rune_map[char]
	if !ok {
		return nil, fmt.Errorf("Rune %c not found in font large bold", char)
	}
	filePath := "sprites/fonts/largebold/" + relPath + ".bmp"

	img, err := LoadSprite(filePath)

	return img, err
}

func Use_Font_Large_Bold(d Driver) map[rune]*image.Image {
	const fontPrefix = "largebold"
	if cache := d.GetFontCache(fontPrefix); cache == nil {
		panic(fmt.Sprintf("Font %s not loaded", fontPrefix))
	} else {
		return cache
	}
}

func Load_Font_Large_Bold(d Driver) {
	const fontPrefix = "largebold"
	mapping := Font_large_bold_rune_map
	mapfunc := load_font_large_bold_rune
	Load_Font_Map(fontPrefix, mapping, mapfunc, d)
}
