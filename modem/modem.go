...
// Modem holds the channels we will use to broadcast events to the rest of our app.
type Modem struct {
	modemmanager.Modem
	DB               *gorm.DB
	OK               bool
...
func Run(debug bool, db *gorm.DB) *Modem {
	// Initialize
	instance := &Modem{
		Debug:                  debug,
		DB:                     db,
		RingingChan:            make(chan *Call),
		calls:                  make(map[dbus.ObjectPath]*Call, 0),
...
func (m *Modem) ReadEmergencyNumbers() {
	if !m.OK {
		return
	}
...
func (m *Modem) SaveSMS(sms modemmanager.Sms, smsPath dbus.ObjectPath) {
	if !m.OK {
		return
	}
	if !m.messaging_ok {
...
func (m *Modem) ProcessNewVoicemail(conn *dbus.Conn, msgPath dbus.ObjectPath) {
...
func (m *Modem) SaveCallStateEvent(call modemmanager.Call, callPath dbus.ObjectPath) {
...
func (m *Modem) SetFlightMode(state bool) {
	if !m.OK {
		return
	}
	var err error
...
func (m *Modem) CheckSignalStrength() {
	if !m.OK {
		return
	}
...
func (m *Modem) GetCarrierAndRoaming() {
	if !m.OK {
		return
	}
...
func (m *Modem) StartAudio(call *Call) {
...
func (m *Modem) ListenToModemEvents() {
...
func (m *Modem) PlaceCall(number string) (*Call, error) {
	if !m.OK {
...
func (m *Modem) AnswerCall(call *Call) error {
	if !m.OK {
		return fmt.Errorf("modem is not ready")
	}

	log.Printf("📞 Answering call at %s...", call.string())
...
func (m *Modem) HangupCall(call *Call) error {
	if !m.OK {
		return fmt.Errorf("modem is not ready")
	}

	log.Printf("📞 Hanging up call %s...", call.string())
...
func (m *Modem) SendDTMF(call *Call, tones string) error {
	if !m.OK {
		return fmt.Errorf("modem is not ready")
	}

	log.Printf("📞 Sending DTMF tones '%s' to call %s...", tones, call.string())
...
func (m *Modem) HoldCall(call *Call) error {
	if !m.OK {
		return fmt.Errorf("modem is not ready")
	}
	conn, err := dbus.SystemBus()
